...
export async function seedN8nSystemWorkflows(): Promise<void> {
    ...
    while (attempts < MAX_RETRIES) {
        try {
            ...
            const isRetriable = error instanceof AppError && (
                error.code === 'N8N_UNAVAILABLE' || 
                error.code === 'N8N_REQUEST_FAILED' || 
                error.message.includes('não encontrado') // Handle 404 during startup
            );

            if (isRetriable && !isLastAttempt) {
                logger.warn({ error, attempt: attempts, nextRetryIn: `${RETRY_DELAY_MS / 1000}s` }, 'n8n not ready or returned 404; retrying seed...');
                await sleep(RETRY_DELAY_MS);
                continue;
            }

            logger.warn({ error, attempts }, 'n8n seed failed after multiple attempts; API will continue without seed.');
            break;
        }
    }
}
